...
namespace CTIN.WebApi.Modules.General.Controllers
{
    ...
    public class InvestsController : ApiController
    {

        private readonly IInvestsService _sv;
        public readonly ICurrentUserService _currentUserService;

        public InvestsController(IInvestsService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_InvestsModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_InvestsModel model)
        {
            var modelService = model.MapToObject<Add_InvestsServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_InvestsModel model)
        {

            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_InvestsServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                var modelService = new Delete_InvestsServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_InvestsModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_InvestsServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_InvestsModel model)
        {
            if (ModelState.IsValid)
            {
