...
namespace CTIN.WebApi.Modules.General.Controllers
{
    public class TempController : ApiController
    {
        ...
        {
            //thoi gian ko duoc la qua khu            
            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Add_TemplateServiceModel>();
                //set value default
        ...
        public async Task<object> Edit(Guid id, [FromBody] Edit_TempModel model)
        {
            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_TemplateServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(Guid id)
        {
            if (ModelState.IsValid)
            {
                var modelService = new Delete_TemplateServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_TempModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOne([FromRoute] Guid id)
        {
            if (ModelState.IsValid)
            {
                var where = JObject.FromObject(new { id }).JsonToString();
                var result = await _sv.FindOne(new FindOne_TemplateServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
