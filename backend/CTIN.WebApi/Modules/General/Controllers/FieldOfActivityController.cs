...
{
    public class FieldOfActivityController : ApiController
    {

        private readonly IFieldOfActivityService _sv;
        public readonly ICurrentUserService _currentUserService;

        public FieldOfActivityController(IFieldOfActivityService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_FieldOfActivityModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_FieldOfActivityModel model)
        {
            var modelService = model.MapToObject<Add_FieldOfActivityServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_FieldOfActivityModel model)
        {

            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_FieldOfActivityServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                var modelService = new Delete_FieldOfActivityServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_FieldOfActivityModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_FieldOfActivityServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_FieldOfActivityModel model)
        {
            if (ModelState.IsValid)
            {
