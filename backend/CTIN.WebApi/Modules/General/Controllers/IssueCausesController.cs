...
using CTIN.WebApi.Modules.IssueCauses.Models;
using Microsoft.AspNetCore.Mvc;
using Newtonsoft.Json.Linq;
using System;
using System.Threading.Tasks;

namespace CTIN.WebApi.Modules.IssueCauses.Controllers
{
    public class IssueCausesController : ApiController
    {
        private readonly IIssueCausesService _sv;
        public readonly ICurrentUserService _currentUserService;
        public IssueCausesController(IIssueCausesService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_IssueCausesModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_IssueCausesModel model)
        {
            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Add_IssueCausesServiceModel>();
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_IssueCausesModel model)
        {
            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_IssueCausesServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            if (ModelState.IsValid)
            {
                var modelService = new Delete_IssueCausesServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_IssueCausesModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOne([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_IssueCausesServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_IssueCausesModel model)
        {
            if (ModelState.IsValid)
            {
