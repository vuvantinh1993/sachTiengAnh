...
{
    public class StorageCabinetsController : ApiController
    {

        private readonly IStorageCabinetsService _sv;
        public readonly ICurrentUserService _currentUserService;

        public StorageCabinetsController(IStorageCabinetsService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_StorageCabinetsModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_StorageCabinetsModel model)
        {
            var modelService = model.MapToObject<Add_StorageCabinetsServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_StorageCabinetsModel model)
        {

            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_StorageCabinetsServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                var modelService = new Delete_StorageCabinetsServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_StorageCabinetsModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_StorageCabinetsServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_StorageCabinetsModel model)
        {
            if (ModelState.IsValid)
            {
