...
using CTIN.WebApi.Modules.WorkType.Models;
using Microsoft.AspNetCore.Mvc;
using Newtonsoft.Json.Linq;
using System;
using System.Threading.Tasks;

namespace CTIN.WebApi.Modules.WorkType.Controllers
{
    public class WorkTypeController : ApiController
    {
        private readonly IWorkTypeService _sv;
        public readonly ICurrentUserService _currentUserService;
        public WorkTypeController(IWorkTypeService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
            _currentUserService = currentUserService;
        }
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_WorkTypeModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_WorkTypeModel model)
        {
                var modelService = model.MapToObject<Add_WorkTypeServiceModel>();
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_WorkTypeModel model)
        {
            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_WorkTypeServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            if (ModelState.IsValid)
            {
                var modelService = new Delete_WorkTypeServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_WorkTypeModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_WorkTypeServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_WorkTypeModel model)
        {
            if (ModelState.IsValid)
            {
