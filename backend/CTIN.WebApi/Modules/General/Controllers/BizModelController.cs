...
{
    public class BizModelController : ApiController
    {

        private readonly IBizModelService _sv;
        public readonly ICurrentUserService _currentUserService;

        public BizModelController(IBizModelService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_BizModelModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_BizModelModel model)
        {
            var modelService = model.MapToObject<Add_BizModelServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_BizModelModel model)
        {

            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_BizModelServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                var modelService = new Delete_BizModelServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_BizModelModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_BizModelServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_BizModelModel model)
        {
            if (ModelState.IsValid)
            {
