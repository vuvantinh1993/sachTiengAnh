...
namespace CTIN.WebApi.Modules.General.Controllers
{
    ...
    public class ContractFormController : ApiController
        {
            private readonly IContractFormService _sv;
            public readonly ICurrentUserService _currentUserService;

            public ContractFormController(IContractFormService sv, ICurrentUserService currentUserService)
            {
                _sv = sv;
            ...
            [HttpGet]
            public async Task<IActionResult> Get([FromQuery] Search_ContractFormModel model)
            {
            ...
            [HttpPost]
            public async Task<object> Add([FromBody] Add_ContractFormModel model)
            {
                var modelService = model.MapToObject<Add_ContractFormServiceModel>();
                //set value default
            ...
            [HttpPut("{id}")]
            public async Task<object> Edit(int id, [FromBody] Edit_ContractFormModel model)
            {

                if (ModelState.IsValid)
                {
                    var modelService = model.MapToObject<Edit_ContractFormServiceModel>();
                    //set value default
                    modelService.id = id;
            ...
            public async Task<object> Delete(int id)
            {
                ...
                if (ModelState.IsValid)
                {
                    var modelService = new Delete_ContractFormServiceModel { id = id };
                    //set value default
            ...
            [HttpGet("FindOne")]
            public async Task<object> FindOne([FromQuery] FindOne_ContractFormModel model)
            {
                if (ModelState.IsValid)
                {
            ...
            public async Task<object> FindOneById([FromRoute] int id)
            {
                if (ModelState.IsValid)
                {
                    ...
                    var result = await _sv.FindOne(new FindOne_ContractFormServiceModel { where = where });
                    return await BindData(result.data, result.errors);
                }
                return await BindData();
            }
            ...
            [HttpGet("Count")]
            public async Task<object> Count([FromQuery] Count_ContractFormModel model)
            {
                if (ModelState.IsValid)
                {
