...
namespace CTIN.WebApi.Modules.General.Controllers
{
    ...
    public class RankController : ApiController
    {
        ...
        public RankController(IRankService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_RankModel model)
        {
        ...
        public async Task<object> Add([FromBody] Add_RankModel model)
        {
            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Add_RankServiceModel>();
                //set value default
        ...
        [HttpPut("{id}")]
        [Authorize(Roles = "Admin")]
        public async Task<object> Edit(int id, [FromBody] Edit_RankModel model)
        {
            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_RankServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            if (ModelState.IsValid)
            {
                var modelService = new Delete_RankServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_RankModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_RankServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_RankModel model)
        {
            if (ModelState.IsValid)
            {
