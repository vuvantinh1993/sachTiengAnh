...
using CTIN.WebApi.Modules.Target.Models;
...
namespace CTIN.WebApi.Modules.Target.Controllers
{
    public class TargetController : ApiController
    {

        private readonly ITargetService _sv;
        public readonly ICurrentUserService _currentUserService;

        public TargetController(ITargetService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_TargetModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_TargetModel model)
        {
            var modelService = model.MapToObject<Add_TargetServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_TargetModel model)
        {

            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_TargetServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                var modelService = new Delete_TargetServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_TargetModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_TargetServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_TargetModel model)
        {
            if (ModelState.IsValid)
            {
