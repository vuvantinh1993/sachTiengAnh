...
namespace CTIN.WebApi.Modules.General.Controllers
{
    ...
    public class TipsController : ApiController
    {
        ...
        public TipsController(ITipsService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_TipsModel model)
        {
        ...
        public async Task<object> Add([FromBody] Add_TipsModel model)
        {
            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Add_TipsServiceModel>();
                //set value default
        ...
        public async Task<object> Edit(int id, [FromBody] Edit_TipsModel model)
        {
            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_TipsServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            if (ModelState.IsValid)
            {
                var modelService = new Delete_TipsServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_TipsModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_TipsServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_TipsModel model)
        {
            if (ModelState.IsValid)
            {
