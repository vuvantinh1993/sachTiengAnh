...
using CTIN.WebApi.Modules.TimeSheetDetail.Models;
using Microsoft.AspNetCore.Mvc;
using Newtonsoft.Json.Linq;
using System;
using System.Threading.Tasks;

namespace CTIN.WebApi.Modules.TimeSheetDetail.Controllers
{
    public class TimeSheetDetailController : ApiController
    {
        private readonly ITimeSheetDetailService _sv;
        public readonly ICurrentUserService _currentUserService;
        public TimeSheetDetailController(ITimeSheetDetailService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_TimeSheetDetailModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_TimeSheetDetailModel model)
        {
            var modelService = model.MapToObject<Add_TimeSheetDetailServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_TimeSheetDetailModel model)
        {
            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_TimeSheetDetailServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            if (ModelState.IsValid)
            {
                var modelService = new Delete_TimeSheetDetailServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_TimeSheetDetailModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_TimeSheetDetailServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_TimeSheetDetailModel model)
        {
            if (ModelState.IsValid)
            {
