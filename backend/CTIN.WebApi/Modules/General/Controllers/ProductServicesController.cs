...
namespace CTIN.WebApi.Modules.General.Controllers
{
    public class ProductServicesController : ApiController
    {
        ...
        private readonly IProductServicesService _sv;
        public readonly ICurrentUserService _currentUserService;

        public ProductServicesController(IProductServicesService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_ProductServicesModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_ProductServicesModel model)
        {
            var modelService = model.MapToObject<Add_ProductServicesServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_ProductServicesModel model)
        {

            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_ProductServicesServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                var modelService = new Delete_ProductServicesServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_ProductServicesModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_ProductServicesServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_ProductServicesModel model)
        {
            if (ModelState.IsValid)
            {
