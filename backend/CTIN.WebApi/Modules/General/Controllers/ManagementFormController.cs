...
{
    public class ManagementFormController : ApiController
    {

        private readonly IManagementFormService _sv;
        public readonly ICurrentUserService _currentUserService;

        public ManagementFormController(IManagementFormService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_ManagementFormModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_ManagementFormModel model)
        {
            var modelService = model.MapToObject<Add_ManagementFormServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_ManagementFormModel model)
        {

            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_ManagementFormServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                var modelService = new Delete_ManagementFormServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_ManagementFormModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_ManagementFormServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_ManagementFormModel model)
        {
            if (ModelState.IsValid)
            {
