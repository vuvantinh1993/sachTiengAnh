...
namespace CTIN.WebApi.Modules.General.Controllers
{
    ...
    {
        private readonly IStatusService _sv;
        public readonly ICurrentUserService _currentUserService;

        public StatusController(IStatusService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
            _currentUserService = currentUserService;
        }
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_StatusModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_StatusModel model)
        {
            var modelService = model.MapToObject<Add_StatusServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_StatusModel model)
        {

            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_StatusServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                var modelService = new Delete_StatusServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_StatusModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_StatusServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_StatusModel model)
        {
            if (ModelState.IsValid)
            {
