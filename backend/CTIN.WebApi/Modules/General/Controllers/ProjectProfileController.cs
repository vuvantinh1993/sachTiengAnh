...
{
    public class ProjectProfileController : ApiController
    {
        private readonly IProjectProfileService _sv;
        public readonly ICurrentUserService _currentUserService;

        public ProjectProfileController(IProjectProfileService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_ProjectProfileModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_ProjectProfileModel model)
        {
            var modelService = model.MapToObject<Add_ProjectProfileServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_ProjectProfileModel model)
        {

            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_ProjectProfileServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                var modelService = new Delete_ProjectProfileServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_ProjectProfileModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_ProjectProfileServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_ProjectProfileModel model)
        {
            if (ModelState.IsValid)
            {
