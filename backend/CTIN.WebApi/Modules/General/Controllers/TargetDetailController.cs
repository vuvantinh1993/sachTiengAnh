...
{
    public class TargetDetailController : ApiController
    {

        private readonly ITargetDetailService _sv;
        public readonly ICurrentUserService _currentUserService;

        public TargetDetailController(ITargetDetailService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_TargetDetailModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_TargetDetailModel model)
        {
            var modelService = model.MapToObject<Add_TargetDetailServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_TargetDetailModel model)
        {

            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_TargetDetailServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                var modelService = new Delete_TargetDetailServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_TargetDetailModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_TargetDetailServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_TargetDetailModel model)
        {
            if (ModelState.IsValid)
            {
