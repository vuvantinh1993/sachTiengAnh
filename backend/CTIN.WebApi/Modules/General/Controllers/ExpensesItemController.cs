...
{
    public class ExpensesItemController : ApiController
    {

        private readonly IExpensesItemService _sv;
        public readonly ICurrentUserService _currentUserService;

        public ExpensesItemController(IExpensesItemService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_ExpensesItemModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_ExpensesItemModel model)
        {
            var modelService = model.MapToObject<Add_ExpensesItemServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_ExpensesItemModel model)
        {

            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_ExpensesItemServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                var modelService = new Delete_ExpensesItemServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_ExpensesItemModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_ExpensesItemServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_ExpensesItemModel model)
        {
            if (ModelState.IsValid)
            {
