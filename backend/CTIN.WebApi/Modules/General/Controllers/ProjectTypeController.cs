...
namespace CTIN.WebApi.Modules.General.Controllers
{
    ...
    {
        private readonly IProjectTypeService _sv;
        public readonly ICurrentUserService _currentUserService;

        public ProjectTypeController(IProjectTypeService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_ProjectTypeModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_ProjectTypeModel model)
        {
            var modelService = model.MapToObject<Add_ProjectTypeServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_ProjectTypeModel model)
        {
            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_ProjectTypeServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            if (ModelState.IsValid)
            {
                var modelService = new Delete_ProjectTypeServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_ProjectTypeModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOne([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                var where = JObject.FromObject(new { and = new List<object> { new { id } } }).JsonToString();
                var result = await _sv.FindOne(new FindOne_ProjectTypeServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_ProjectTypeModel model)
        {
            if (ModelState.IsValid)
            {
