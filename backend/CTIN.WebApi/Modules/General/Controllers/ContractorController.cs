...
using CTIN.WebApi.Modules.Contractor.Models;
using Microsoft.AspNetCore.Mvc;
using Newtonsoft.Json.Linq;
using System;
using System.Threading.Tasks;

namespace CTIN.WebApi.Modules.Contractor.Controllers
{

    public class ContractorController : ApiController
    {
        private readonly IContractorService _sv;
        public readonly ICurrentUserService _currentUserService;

        public ContractorController(IContractorService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_ContractorModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_ContractorModel model)
        {
            var modelService = model.MapToObject<Add_ContractorServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_ContractorModel model)
        {

            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_ContractorServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                var modelService = new Delete_ContractorServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_ContractorModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_ContractorServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_ContractorModel model)
        {
            if (ModelState.IsValid)
            {
