...
namespace CTIN.WebApi.Modules.General.Controllers
{
    ...
    {
        private readonly IContractsService _sv;
        public readonly ICurrentUserService _currentUserService;

        public ContractsController(IContractsService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        /// <summary>
        /// lấy dữ liệu Bảng Contracts - Quản lý hợp đồng
        /// </summary>
        /// <param name="model">
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_ContractsModel model)
        {
        ...
        /// <summary>
        /// thêm dữ liệu Bảng Contracts - Quản lý hợp đồng
        /// </summary>
        /// <param name="model">
        ...
        /// </returns>
        [HttpPost]
        public async Task<object> Add([FromBody] Add_ContractsModel model)
        {

            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Add_ContractsServiceModel>();
                //set value default
        ...
        /// <summary>
        /// sửa dữ liệu Bảng Contracts - Quản lý hợp đồng
        /// </summary>
        ...
        /// </returns>
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_ContractsModel model)
        {
            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_ContractsServiceModel>();
                //set value default
                modelService.id = id;
                modelService.modifiedBy = _currentUserService.userId;
                modelService.modifiedDate = DateTime.Now;
        ...
        /// <summary>
        /// Chỉnh sửa một số dữ liệu Bảng Contracts - Quản lý hợp đồng
        /// </summary>
        /// <param name="id"> là id của đối tượng cần chỉnh sửa </param>
        ...
        /// <summary>
        /// Xóa dữ liệu Bảng Contracts theo id - Quản lý hợp đồng
        /// </summary>
        ...
        public async Task<object> Delete(int id)
        {
            if (ModelState.IsValid)
            {
                var modelService = new Delete_ContractsServiceModel { id = id };
        ...
        /// <summary>
        /// Tìm dữ liệu đầu tiên phù hợp model của Bảng Contracts - Quản lý hợp đồng
        /// </summary>
        ...
        /// </returns>
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_ContractsModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneByID([FromRoute] long id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_ContractsServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        /// <summary>
        /// Đếm số row Bảng Contracts theo điều kiện model - Quản lý hợp đồng
        /// </summary>
        /// <param name="model"> là biểu thức điều kiện Where truyền lên</param>
        /// <param>
        ///     Kiểm tra ModelState xem có hợp lệ không
        ...
        /// </returns>
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_ContractsModel model)
        {
            if (ModelState.IsValid)
            {
