...
{
    public class ExpensesItemGroupController : ApiController
    {

        private readonly IExpensesItemGroupService _sv;
        public readonly ICurrentUserService _currentUserService;

        public ExpensesItemGroupController(IExpensesItemGroupService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_ExpensesItemGroupModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_ExpensesItemGroupModel model)
        {
            var modelService = model.MapToObject<Add_ExpensesItemGroupServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_ExpensesItemGroupModel model)
        {

            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_ExpensesItemGroupServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                var modelService = new Delete_ExpensesItemGroupServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_ExpensesItemGroupModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_ExpensesItemGroupServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_ExpensesItemGroupModel model)
        {
            if (ModelState.IsValid)
            {
