...
using CTIN.WebApi.Modules.BiddingModel.Models;
using Microsoft.AspNetCore.Mvc;
using Newtonsoft.Json.Linq;
using System;
using System.Threading.Tasks;

namespace CTIN.WebApi.Modules.BiddingModel.Controllers
{
    public class BiddingModelController : ApiController
    {
        private readonly IBiddingModelService _sv;
        public readonly ICurrentUserService _currentUserService;
        public BiddingModelController(IBiddingModelService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_BiddingModelModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_BiddingModelModel model)
        {
            var modelService = model.MapToObject<Add_BiddingModelServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_BiddingModelModel model)
        {
            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_BiddingModelServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            if (ModelState.IsValid)
            {
                var modelService = new Delete_BiddingModelServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_BiddingModelModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_BiddingModelServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_BiddingModelModel model)
        {
            if (ModelState.IsValid)
            {
