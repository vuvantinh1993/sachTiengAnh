...
{
    public class TermsOfPaymentController : ApiController
    {
        private readonly ITermsOfPaymentService _sv;
        public readonly ICurrentUserService _currentUserService;

        public TermsOfPaymentController(ITermsOfPaymentService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_TermsOfPaymentModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_TermsOfPaymentModel model)
        {
            var modelService = model.MapToObject<Add_TermsOfPaymentServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_TermsOfPaymentModel model)
        {

            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_TermsOfPaymentServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                var modelService = new Delete_TermsOfPaymentServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_TermsOfPaymentModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_TermsOfPaymentServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_TermsOfPaymentModel model)
        {
            if (ModelState.IsValid)
            {
