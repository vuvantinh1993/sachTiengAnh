...
{
    public class CrtypeController : ApiController
    {
        private readonly ICrtypeService _sv;
        public readonly ICurrentUserService _currentUserService;

        public CrtypeController(ICrtypeService sv, ICurrentUserService currentUserService)
        {
            _sv = sv;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] Search_CrtypeModel model)
        {
        ...
        [HttpPost]
        public async Task<object> Add([FromBody] Add_CrtypeModel model)
        {
            var modelService = model.MapToObject<Add_CrtypeServiceModel>();
            //set value default
        ...
        [HttpPut("{id}")]
        public async Task<object> Edit(int id, [FromBody] Edit_CrtypeModel model)
        {

            if (ModelState.IsValid)
            {
                var modelService = model.MapToObject<Edit_CrtypeServiceModel>();
                //set value default
                modelService.id = id;
        ...
        public async Task<object> Delete(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                var modelService = new Delete_CrtypeServiceModel { id = id };
                //set value default
        ...
        [HttpGet("FindOne")]
        public async Task<object> FindOne([FromQuery] FindOne_CrtypeModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<object> FindOneById([FromRoute] int id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _sv.FindOne(new FindOne_CrtypeServiceModel { where = where });
                return await BindData(result.data, result.errors);
            }
            return await BindData();
        }
        ...
        [HttpGet("Count")]
        public async Task<object> Count([FromQuery] Count_CrtypeModel model)
        {
            if (ModelState.IsValid)
            {
